...
- 本項作業取代傳統SURFER、[NCL][ncl]、或[dat2kml][dat2kml]、[csv2kml][csv2kml]等繪圖方式，以python程式讀取高斯煙流模式結果(plotfile、格式詳下)，運作[NCL][ncl]並讀取結果檔，與[Open TopoMap][OTM]地圖進行疊加，並以[CaaS][caas]形式對外提供遠端計算服務。
  - 正確、迅速取得指定範圍、指定解析度之OSM/[OTM][OTM]底圖
  - 應用[NCL][ncl]製作報告品質之等值線圖
  - 將此二者予以疊加、提供使用者下載
...
|:--:|
| <b>遠端計算服務：在[OpenTopoMap][OTM]上貼上高斯煙流模式之[NCL][ncl]等值圖</b>| 

## CaaS成果及運作
### 輸入說明：
- (一) RE GRIDCART
  - 在空格內至少要輸入6個數字，數字的內容、順序及意義如[下表]()所示(ISC/AERMOD之受體點設定方式)，其他考量檢討如下：
  - 直角座標系統的必要性
...
- (二)PLT檔案
  - 必須是單一網格系統、此處的[NCL][ncl]程式並不會重新進行regrid
    - 目前[NCL][ncl]程式尚不接受加入離散點、
    - X、Y方向的間距「必須」相等
    - 可接受不同方向的格點數不同(nx!=ny)
  - 因為煙流模式目前尚無法區分網格系統或離散點分別輸出成PLT檔，可行因應的方式包括：
    1. 先使用[dat2kml][dat2kml]產生.grd檔
    1. 刪除煙流模式中有關其他網格系統、離散點的設定、重新執行煙流模式。
    1. 另在python程式內做regard，輸出等間距網格的結果。
    1. 修改NCL程式、重新進行REGRID


### OTM檔案下載、合併與切割（ tiles_to_tiffFit.py）
本項工作的目標是得到指定範圍、指定解析度之OTM檔案，可以給SURFER或其他繪圖軟體使用。
詳細參閱詳merged_GeoTIFF及筆記之說明，此處說明搭配的原則與注意事項：
- (一)引數個數與內容的應用
...
- (四)合併(merge)或拼接(montage)
  ...
  - 衛星圖像不會變色
    ...
    - 解決方案
      - 使用OSMosis(未執行)
        - 使用montage拼接：唯一要修改檔案命名方式，原本x-y-z的檔名，轉變成z-y-x，以利程式按照各圖所在位置直接拼接
      - 使用批次檔mtg.cs
...
### NCL等值線製圖
本項工作的目標是得到像素較高的等值線圖。使用NCL的理由：
- 品質為科學期刊所認可、軟體持續精進更新、備援及範例豐富
- Linux/macos相容、自動化作業
- 出圖像素可控制
- (一)矩陣大小的判定
  - NCL 的asciiread可以接受未知維度、長度的檔案，但是要在NCL程式內判斷檔案X軸及Y軸的格點數，因為沒有set()指令，困難度太高了、只能放棄
  - 在cgi_python內呼叫os.system("echo”)指令，將nx,ny等設定參數寫在檔案param.txt內(只有用到網格數)
  - 在NCL內讀入維度、宣告檔案及變數的陣列大小、順利將矩陣讀入
...
### NCLonOTM.html
設計成可以接受字串與檔案輸入。右側有成品範例、下側有設定數字意義表格。

### cgi_python
主要工作就是接受html的呼叫，將座標設定內容或檔案內容傳遞到前述3項作業。
- (一)按照輸入字串呼叫FIT程式
...
```
- (二)將regridded的SURFER grd檔案寫成tmp.PLT (X,Y,C)格式
  - 判斷邏輯是第一行、第一個字串必須是“DSAA”
  - 使用網友提供的load_surfer模組
  - 寫出時要注意python2還沒有{:f}格式
...
```python
    ...
    70                f.write('%f %f %f\n' % (x[j,i],y[j,i],c[j,i]))
```
- (三)如是PLOTFILE格式，還需要進行SED去行、CUT選欄位
  - tmp.PLT檔案整理好之後就可以進行FIT程式

```python
    14    CUT=' |cut -c 1-44 >& tmp.PLT'   
...
    22    SED='/usr/bin/sed "1,8d" '
...
...
- NCAR graphic Command Language、NCL
- Open Topo Map OTM
  - Open Street Map processors, OSMOSIS
- Jimmy Utterström,  Generate merged GeoTIFF imagery from web maps (xyz tile servers) with Python, merged_GeoTIFF
- SURFER 
  - automation
  - georeferencing
- create a montage from images in Linux using ImageMagick suite,  montage
